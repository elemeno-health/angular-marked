...
function markedDirective(marked, $templateRequest, $compile) {
  return {
    ...
    link: function (scope, element, attrs) {
      if (attrs.marked) {
        set(scope.marked, scope.skipAccordionize);
        scope.$watch('marked', set);
      } else if (attrs.src) {
        scope.$watch('src', function (src) {
          $templateRequest(src, true).then(function (response) {
            set(response, scope.skipAccordionize);
          }, function () {
            set('', scope.skipAccordionize);
            scope.$emit('$markedIncludeError', attrs.src);
          });
        });
      } else {
        set(element.text(), scope.skipAccordionize);
      }

      function set(text, skipAccordionize) {
        text = unindent(String(text || ''));

        var markedText = marked(text, scope.opts || null)

        if (typeof scope.skipAccordionize !== 'undefined') {
          element.html(markedText);
        } else {
          element.html(accordionize(markedText));
        }

        if (scope.$eval(attrs.compile)) {
          $compile(element.contents(), skipAccordionize)(scope.$parent);
        }
      }

      function accordionize(markedStr) {
        // Create nodes from marked string
        var oldHtml = document.createElement('div');
        oldHtml.innerHTML = markedStr;

        // Remove old h1
        var h1 = oldHtml.querySelector('h1');
        if (h1) oldHtml.removeChild(h1);

        // Build accordion
        var ctAccordion = document.createElement('ct-accordion');
        ctAccordion.setAttribute('class', 'markdown-accordion');
        ctAccordion.setAttribute('title', h1 && h1.innerText || '');

        // Build accordion panels
        var children = oldHtml.childNodes;
        var newChildren = [];
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          if (currentChild.tagName === 'H2') {
            // close panel if one is already open
            if (currentPanel !== null) {
              newChildren.push(currentPanel);
              currentPanel = null;
            }

            // h2 opens accordion panel
            var newChild = document.createElement('ct-acc-panel');
            newChild.setAttribute('class', 'markdown-panel');
            newChild.setAttribute('title', currentChild.innerText);
            currentPanel = newChild;
          } else {
            // clone node before appending
            var currentChildCopy = currentChild.cloneNode(true);
            if (currentPanel) {
              currentPanel.appendChild(currentChildCopy);
            } else {
              newChildren.push(currentChildCopy);
            }
          }
        }
        ...
        // Append accordion children
        for (var n = 0; n < newChildren.length; n++) {
          ctAccordion.appendChild(newChildren[n]);
        }

        return ctAccordion.outerHTML;
      }
    }
  };
}
